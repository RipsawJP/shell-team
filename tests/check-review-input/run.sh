...
assert_case "second section with no block refuses section-incomplete" 1 section-incomplete -- bash "$SCRIPT" --record "$T/gap.md" --task T-000
...
if printf '%s' "$out2" | grep -Fq -- "$MARKER"; then
  fail "no-echo case 2: the marker leaked into output"
fi
pass "no-echo case 2: marker never echoed (collision path)"
