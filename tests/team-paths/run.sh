...
# --- total-key set is exactly nine, in both directions (T-1002 AC15) --------
# A tenth key added without updating this list fails it; a key silently
# dropped from --export/--print (which count independently) fails it too.
for k in base todo loops runs retros reviews specs provenance interventions; do
  bash "$PATHS" --root "$D" --get "$k" >/dev/null \
    || fail "total-key set is exactly nine: key '$k' should resolve"
done
set +e
bash "$PATHS" --root "$D" --get lessons >/dev/null 2>&1
rc_unlisted=$?
set -e
[ "$rc_unlisted" -eq 2 ] || fail "total-key set is exactly nine: an unlisted key should exit 2, got $rc_unlisted"
export_count="$(bash "$PATHS" --root "$D" --export | grep -c -- '^export TEAM_')"
[ "$export_count" -eq 9 ] || fail "total-key set is exactly nine: --export should print exactly 9 lines, got $export_count"
print_count="$(bash "$PATHS" --root "$D" --print | grep -cE -- '^[[:space:]]+[a-z]+[[:space:]]+[^[:space:]]+$')"
[ "$print_count" -eq 9 ] || fail "total-key set is exactly nine: --print should show exactly 9 rows, got $print_count"
pass "total-key set is exactly nine: every key resolves, an unlisted key exits 2, --export/--print each carry exactly nine entries"
