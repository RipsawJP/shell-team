...
check-handoff.sh:21:  printf '%s: cannot read file\n' "$FILE" >&2
check-handoff.sh:71:  printf '%s:%s: %s: %s\n' "$FILE" "$1" "$2" "$3" >&2
...
gen-playbook-blocks.sh:466:        "$role" "$line_count" "$LINE_WARN_THRESHOLD" >&2
EOF
...
# exempted NOT_APPLY line (close-out.sh:258) and simulate REWRITING its TEXT
...
sed 's|^close-out\.sh:258:.*$|close-out.sh:258:  printf '"'"'MUTATED: this text simulates an exempted line rewritten unsafe'"'"' >&2; exit 42|' \
...
if grep -qF 'close-out.sh:258:' "$MUTATION_IN_SCOPE_FILE"; then
  ok "mutation self-check: rewriting the close-out.sh:258 NOT_APPLY line's TEXT (same file:line) to an unsafe form IS caught by the content-aware audit (would NOT have been caught by a file:line-only match)"
else
  bad "mutation self-check: rewriting the close-out.sh:258 NOT_APPLY line's TEXT to an unsafe form was NOT caught — the content-aware audit is not actually content-aware; every completeness result above is suspect"
fi
