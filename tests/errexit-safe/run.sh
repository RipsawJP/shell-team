...
close-out.sh:264:  printf 'close-out: note: project_status generated block not refreshed (file or markers absent) — see gen-project-status.sh\n' >&2
...
# exempted NOT_APPLY line (close-out.sh:264) and simulate REWRITING its TEXT
...
sed 's|^close-out\.sh:264:.*$|close-out.sh:264:  printf '"'"'MUTATED: this text simulates an exempted line rewritten unsafe'"'"' >&2; exit 42|' \
...
if grep -qF 'close-out.sh:264:' "$MUTATION_IN_SCOPE_FILE"; then
  ok "mutation self-check: rewriting the close-out.sh:264 NOT_APPLY line's TEXT (same file:line) to an unsafe form IS caught by the content-aware audit (would NOT have been caught by a file:line-only match)"
else
  bad "mutation self-check: rewriting the close-out.sh:264 NOT_APPLY line's TEXT to an unsafe form was NOT caught — the content-aware audit is not actually content-aware; every completeness result above is suspect"
fi
