---
description: Run a /goal self-verification loop — repeatedly implement+verify a single task until a layered completion gate (check-acs → check-intent, only when the spec carries a frozen intent block → check-provenance → qa-verifier → code-reviewer) is fully green, bounded by loop-guard.sh (iteration / wall-clock / no-progress). Use when you want the team to drive a task to "done" on its own cadence rather than one manual pass.
---
...
**Step 0 — resolve paths & contract.** Run `team-paths.sh --print` once (invoked as `bash "<plugin root>/bin/<script>"` — never assumed to be on `PATH`, even with the plugin loaded; read `<plugin root>` from what your host reports, see `docs/adopting.md`'s "Locate the installed plugin root" step for how, applied here to `team-paths.sh`) to read `TEAM_TODO`,
...
(`engineer` / `qa-verifier` / `code-reviewer`, as `/shell-team:run` does) telling it to
...
(invoked as `bash "<plugin root>/bin/goal-state.sh"` — never assumed to be on `PATH`, even with the plugin loaded; read `<plugin root>` from what your host reports, see `docs/adopting.md`'s "Locate the installed plugin root" step for how.) Skip this on
...
2. **Completion gate (layered, each verifier independent)** — run in order, and
   **do not carry one verifier's conclusion into the next** (a QA false-positive
   ...
   - **Per-tick failure-class tracking (T-058)**: on every non-green tick,
     ...
     all as phase `validate`, `code-reviewer` findings as phase `review` — and keep
...
4. **Decide**:
   - Gate **green** → the loop **succeeds**; report done (board at
     ...
     gate).
   - **Fast-follow disposition recording (T-068)**: before reporting done at `READY_FOR_MERGE`, act on `code-reviewer`'s `#### Fast-follow disposition` declaration — the reviewer states intent only (file-an-issue or won't-fix) and never opens issues or edits the board, so **you (the orchestrator) are the actor**: for a file-an-issue intent, open the issue (via this repo's normal issue-creation flow, user-approval gate where it applies) and capture its number; for a won't-fix intent, capture the reason. Transcribe the result onto the task's board entry as a sub-bullet anchored `- fast-follow disposition (YYYY-MM-DD): …`, using the same closed disposition set as `/shell-team:run`'s Review step — `filed as issue #N`, `waived: <reason>`, or a time-bound `pending: <reason> — <deadline>` that must resolve to `filed as issue #N` or `waived: <reason>` before close-out (`pending` must never survive close-out — `bin/close-out.sh` fails closed if one remains). If the reviewer declared `no fast-follow deferrals`, no line is required. The reviewer does not edit the board — a rule it follows, carried in its own board-and-write-boundary Rule in `agents/code-reviewer.md`, and not a capability it lacks: its `tools:` line grants `Bash`, so it could write the board and does not. Filing and recording are the orchestrator's job, so a surfaced deferral never silently vanishes (the T-066 record gap). The closed set now includes a third terminal value, `deferred: <reason>`, used when the author has not ruled on filing or waiving and close-out proceeds without that ruling; as for `waived:`, the reason is mandatory. List every such `deferred:` finding — together with any other undispositioned fast-follow — under an `undispositioned fast-follows` heading in the hand-off summary and the merge-GO presentation, worded to read correctly for one finding or for several; when none were recorded, the heading is omitted rather than printed empty. A `pending:` already recorded stays unresolved to `deferred:` — it resolves only to `filed as issue #N` or `waived: <reason>`.
   - loop-guard prints **`STOP:<reason>`** (`max_iterations_reached` /
     `budget_exhausted` / `no_progress` / `guard_error`) → **stop** and report the
     reason verbatim to the user — unless the means-ends reflection's disposition
     already executed for this STOP firing (T-1095): then skip this report, since
     the executed disposition and its four recorded answers are themselves the
     loop's response.
     - **Rework-history digest (T-058)**: when escalating a `STOP:`, run `rework-digest.sh` with the per-round root-cause classes you have been tracking across rounds — one `--round <n> --phase <validate|review> --class <slug>` triple per classified rework finding — plus `--stop-reason <reason>` (invoked as `bash "<plugin root>/bin/rework-digest.sh"` — never assumed to be on `PATH`, even with the plugin loaded; read `<plugin root>` from what your host reports, see `docs/adopting.md`'s "Locate the installed plugin root" step for how), and paste its stdout into the escalation message. It prints the per-round failure-class list and the same-class-repetition vs new-classes judgment the human needs for the extend/stop decision. Do not re-describe the digest format by hand — the script is its single source of truth.
...
```
log-run.sh goal --run-id <run_id> --seq <n> --span <engineer|qa-verifier|code-reviewer> \
...
- **Reuse, don't modify** the primitives (`loop-guard.sh`, `check-acs.sh`,
  `log-run.sh`, `goal-state.sh`, the `qa-verifier`/`code-reviewer` agents).
