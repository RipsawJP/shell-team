...
- **Specs**: `<specs dir>/T-XXX-<slug>.md`, slug = kebab-case of the title, where `<specs dir>` is `team-paths.sh --get specs` (invoked as `bash "<plugin root>/bin/team-paths.sh"` — never assumed to be on `PATH`, even with the plugin loaded; read `<plugin root>` from what your host reports, see `docs/adopting.md`'s "Locate the installed plugin root" step for how)
- **Review artifacts**: `<reviews dir>/T-XXX.md` (curated verdict + severity ledger) plus the raw Codex traces `<reviews dir>/T-XXX-codex-*.{txt,jsonl}`, where `<reviews dir>` is `team-paths.sh --get reviews` (same invocation note as above)
