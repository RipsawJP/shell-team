...
- **Specs**: `<specs dir>/T-XXX-<slug>.md`, slug = kebab-case of the title, where `<specs dir>` is `bin/team-paths.sh --get specs`
- **Review artifacts**: `<reviews dir>/T-XXX.md` (curated verdict + severity ledger) plus the raw Codex traces `<reviews dir>/T-XXX-codex-*.{txt,jsonl}`, where `<reviews dir>` is `bin/team-paths.sh --get reviews`
