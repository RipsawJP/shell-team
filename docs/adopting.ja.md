...
一様な話ではない——紐付けられた adapter 自身の `applies-effort` 宣言が、
resolved な値がその呼び出しに実際に届くかどうかをそれぞれ決めており、
このプロジェクトが出荷する 2 つの adapter は互いに異なる挙動をとる：
`codex-cli` の行では値がその呼び出し自体に
**適用**される（`-c model_reasoning_effort=<value>` として、その adapter
自身の `cli-config-override` 宣言に紐付く）。一方 `claude-cli` の行では
span に**記録**されるだけで何にも適用されず、他に及ぶ影響は上記の
`capability-unsupported` refusal だけである(#420)。最後に、どの **executor**（provider と
