...
8. **Claude 側のレビュー pass が必要とする sandbox network access を
   ...
   **通っていた**のに、セッションの**内側**でレビュー pass が同一の
   ...
   二択診断を適用済みの `BLOCKED` レポートで停止するようになった——
   手順を手動でやり直す前に、そのレポートが示す原因と対処法を読むこと。
...
--print-grant` を実行して読む。Claude Code host では、これまでどおり
自分自身の checkout の `CLAUDE.local.md` に記録することもできる。
このプロジェクトはどちらの surface の grant の文面も出荷物には転記せず、
代わりに発明することもしない。
