...
行番号・一致したパターンからなる行が出力される場合——そのパターンを読んでください。
`.shell-team/` のような通常のパターンなら base dir は無視されており、`!` で始まる
否定パターンなら re-include が効いていて無視されていません。何も出力されない場合
——base dir に一致するパターンは無く、無視しているものは何もありません。`fatal:`
メッセージで失敗する場合——git はそもそもこの問いに答えられていません。リポジトリ
が無い、bare リポジトリである、git が読めない、のいずれかであり、この場合はどちら
とも確定していません。
