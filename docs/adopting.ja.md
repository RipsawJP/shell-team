...
で走らせる lint/format/static のステップも**併せてカバーされる**——QA は
それらをローカルで、ワークフローが固定するバージョンの下で実行し（何も
固定していなければその旨を記録する）、非ゼロ終了は round の FAIL になる
（プルリクエストを初めて push した時の驚きではなく）。これは recipe の
状態に**依存しない**——QA は記入済みセクションを待つのではなく、ワークフロー
ファイル自体を読むので、スキャフォールドしたばかりの repository でも最初の
タスクからカバーされる。
...
recipe の `## CI parity` セクションは**前提条件ではなく記録**である——
ワークフローファイル・そこから抜き出した lint/format コマンド・固定された
バージョンを書き留める場所であり、engineer か QA が初回利用時に埋めることで、
後続の round が同じリストをゼロから導出し直さずに済む。
