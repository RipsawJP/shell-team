...
- One disclosed departure from T-1083's own overlap-margin idiom, stated rather than glossed: T-1083 stated its margin against the *maximum single launch latency* (`first − launched-epoch`). This launch record's own `- launched-epoch:` field is disclosed above (evidence disclosure 1) to be an audit-only stamp that **precedes** several instances' own `first` timestamps by design, so `first − launched-epoch` is negative for those instances and cannot supply a meaningful latency bound here. In its place, the margin is stated against the instances' own launch dispersion — the spread between the earliest and latest `first` stamp across all eight instances, ≈34.495 s — giving a ratio of `878198706000 / 34494850000 ≈ 25.5×`, comfortably larger than a bare launch-jitter margin and independent of the disclosed audit-field limitation.
...
- licence-condition: genuine-overlap — met — re-derived above: a non-empty window of `878198706000` ns (≈878.199 s) during which all eight fanned instances were simultaneously executing, against a launch-dispersion spread of `34494850000` ns (≈34.495 s) — a margin ratio of ≈25.5×. The margin is stated against launch dispersion rather than against `first − launched-epoch` (T-1083's own idiom) because this launch record's own `- launched-epoch:` field is disclosed (evidence disclosure 1) to precede several instances' own `first` stamps, making that latency computation negative and meaningless for several instances — a disclosed departure from the sibling note's shape, not a suppressed one.
...
- saving: default-path-tier1-fanout — measured — the fanned arm's own launch record gives `- launched-epoch: 1787185735135713000` and `- completed-epoch: 1787187556288212000` (duration `1821152499000` ns ≈ 1821.152 s); the serial arm's own `- agent-timestamp: t1085serial — serial-1 —` line gives `first=1787196593684218000` and `last=1787199945017994000` (duration `3351333776000` ns ≈ 3351.334 s). `(3351333776000 − 1821152499000) / 3351333776000 ≈ 45.66%` wall-clock reduction on this task's own full 90-unit population at N=8, measured on this one machine in this one session — **not** `docs/loop-engineering/phase-multiplexing.md`'s own **43.84%** pilot figure (`docs/loop-engineering/phase-multiplexing.md` line 355), which stays the claim ceiling this measurement is checked against, never restated as this task's own result.
- arm-wall-clock: fanned — start=1787185735135713000 — end=1787187556288212000
- arm-wall-clock: serial — start=1787196593684218000 — end=1787199945017994000
- saving-direction: faster — the serial arm's own duration (`3351333776000` ns) exceeds the fanned arm's own duration (`1821152499000` ns), so the fanned arm finished faster — the sign the `- saving:` line above reports, re-derived here independently by bash `10#` integer arithmetic rather than accepted from the narrative: `serial_dur=3351333776000; fanned_dur=1821152499000; if [ "$serial_dur" -gt "$fanned_dur" ]; then echo faster; elif [ "$serial_dur" -lt "$fanned_dur" ]; then echo slower; else echo equal; fi` → `faster`.
...
- Sub-second overlap discrimination: the overlap claim this note makes (`## Default-path chain and aggregation analysis` above) is a non-empty intersection window with a stated margin against the instances' own launch dispersion, never a claim at finer timing resolution (Input space).
