...
- unobserved: whether the resolved-type differential T-1111 measured for one plugin role generalizes beyond `subagent_type=shell-team:qa-verifier`, `model=sonnet`, this harness version and this machine — and, independent of role, whether any aspect of role resolution beyond the effective tool set (the model actually bound, the system prompt actually loaded, or behaviour not expressed through the tool set) resolves as declared; `docs/loop-engineering/plugin-role-agent-concurrency.md` measured the general-purpose-vs-plugin-role overlap and write-artifact differential this line previously scoped as unmeasured, and carries the narrower residual's own evidence and `- evidence-boundary:` line.
...
- Agent population: every probe agent this round was `subagent_type=general-purpose`, `model=haiku`; whether the result generalizes to a different model or a different general-purpose-class agent type remains unobserved by this task, and the plugin-role case is now measured separately by `docs/loop-engineering/plugin-role-agent-concurrency.md` (`concurrent-overlapping`, `subagent_type=shell-team:qa-verifier`) — see `## Agent-type boundary` above and that note's own `- evidence-boundary:` line for what its differential does and does not reach.
