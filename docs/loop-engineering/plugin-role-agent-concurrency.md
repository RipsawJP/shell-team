...
**Evidence versioning, disclosed here rather than left implicit.** This note's `## Probe evidence` section below is its **second and final permitted version** under the frozen invalid-evidence rule. The first probe's evidence (committed at `2fa4ccd0a69a8a0bd26cf48fc1f51d902911bed8`) was declared invalid for a recorded reason — its evidence section was drafted without this spec's own **AC4** required `- command: ` line grammar inside the evidence section itself, a conformance omission by the evidence's own author rather than a defect in the frozen protocol or a fact this note invents. The operator explicitly rejected a proposed gate-edit (narrowing **AC4**'s own check to accept the restated commands elsewhere in this note) as itself the result-fitting the freeze-then-probe design exists to forbid (`.shell-team/interventions/T-1111.md`, fifth entry); the disposition taken instead was **repair the work, not the gate** — re-run the identical frozen protocol end to end in a fresh clone, with the evidence drafted to the frozen grammar this time. No byte of the frozen protocol, this spec, or **AC4** moved. The commands the frozen protocol names are tagged directly inside `## Probe evidence` below (four `- command: ` lines, satisfying **AC4**'s evidence-scoped requirement); this section no longer restates them, so a reader sees each command exactly once, at its evidence-section source.
...
**Figures below are re-derived from the v2 evidence committed at `074ee2f67208c43416aa66c63f8545fcf74c29ef`, superseding every v1-derived figure this section previously carried (the v1 probe, evidence `2fa4ccd0a69a8a0bd26cf48fc1f51d902911bed8`, was declared invalid — `## Probe protocol` above).**
...
- overlap: resolved-type-n3-rep1 — overlapping — margin_ns=249686021000 — `min(last)`=1788193993353037000 (`qa-1`), `max(first)`=1788193743667016000 (`qa-3`); ≈249.7 s.
- overlap: control-gp-n3 — overlapping — margin_ns=269546916000 — `min(last)`=1788194338195079000 (`gp-1`), `max(first)`=1788194068648163000 (`gp-3`); ≈269.5 s.
- overlap: resolved-type-n3-rep2 — overlapping — margin_ns=270647473000 — `min(last)`=1788194680215343000 (`qa-3`), `max(first)`=1788194409567870000 (`qa-1`); ≈270.6 s.
...
- batch-vs-sum: resolved-type-n3-rep1 — batch_ns=328548362000 — sum_ns=810598582000 — batch far below sum (≈40.5%), necessary-condition channel agrees with the overlap line above.
- batch-vs-sum: control-gp-n3 — batch_ns=340362469000 — sum_ns=844558452000 — batch far below sum (≈40.3%), agrees.
- batch-vs-sum: resolved-type-n3-rep2 — batch_ns=342264681000 — sum_ns=838145689000 — batch far below sum (≈40.8%), agrees.

**Timing preconditions, re-derived rather than declared:**

- unit-duration: 200344336000 — `end`(1788193698146747000) − `start`(1788193497802411000) from the `- unit-timing: population-fixation` line above; ≈200.3 s.
- launch-latency: 39929979000 — the maximum, over every executed arm, of `max(first) − batch_start`: rep1 ≈39.9 s (the maximum this time, from `- batch-timestamp: resolved-type-n3-rep1`), control ≈36.2 s, rep2 ≈36.7 s.
- margin-factor: 3 — the frozen protocol's declared floor (a policy choice, not a measurement): `unit-duration` (200344336000 ns) ÷ `launch-latency` (39929979000 ns) ≈ 5.02×, comfortably clearing the floor even against the worst-case (this time, rep1-arm) latency.
- clock-source: python3-time_ns (`python3 -c 'import time; print(time.time_ns())'`) — verified-expanding — every timestamp above expanded to a 19-digit integer before reliance (`## Probe evidence`, header paragraph, sample `1788193486828506000`); no BSD-`date`-style literal `N` occurred, since the orchestrator and all nine instances used the python source uniformly, and every recorded `last` exceeds its own `first` with no negative delta across all nine `- agent-timestamp:` lines.

**Launch order, verified arithmetically rather than reported**: the recorded `batch_start` integers read `resolved-type-n3-rep1`(1788193703737037000) < `control-gp-n3`(1788194032416344000) < `resolved-type-n3-rep2`(1788194372909625000) — the frozen order held in practice, so the control arm's between-subjects position is data this note carries, not a claim resting on the frozen protocol's intent alone.

**Discriminator: write-artifact existence versus self-report, per `(arm, instance)` pair.** Every `control-gp-n3` artifact (`gp-1`, `gp-2`, `gp-3`) reads `present`; every subject-arm artifact, across both `resolved-type-n3-rep1` and `resolved-type-n3-rep2` (six instances total), reads `absent`. The consistency coupling holds for all nine pairs: `present` ↔ `write_tool=available` (the three control instances); `absent` ↔ `write_tool=refused` or `write_tool=absent` (all six subject instances — this time unanimously `absent`, unlike v1's one `refused` outlier which did not recur, itself further corroborating evidence that the self-report channel is agent-authored interpretation rather than a harness fact). No `(arm, instance)` pair disagrees, so no invalid-evidence event is recorded. `agents/qa-verifier.md` grants `tools: Read, Grep, Glob, Bash` and no `Write`; a `general-purpose` instance holds one — the within-venue contrast (control `present`, subject `absent`, one shared clone, `probe-write-<arm-id>-<instance-id>.txt` per instance) is exactly the differential this task's design licenses, and a venue-wide write denial cannot produce it, because such a denial would take the control artifacts with it too.

## Verdict and licence conditions

- verdict: concurrent-overlapping — every executed arm (`resolved-type-n3-rep1`, `control-gp-n3`, `resolved-type-n3-rep2`) shows a genuine, large, positive overlap margin (≈249.7 s, ≈269.5 s, ≈270.6 s respectively), each exceeding the maximum single-instance launch latency (≈39.9 s) by more than 6×; the necessary-condition channel agrees at every arm (batch far below sum); all eight licence conditions below read `met`; and the frozen launch order is independently verified from the recorded `batch_start` integers above.
...
- licence-condition: overlap-margin-exceeds-launch-latency — met — `unit-duration` (200344336000 ns) exceeds `margin-factor`(3) × `launch-latency`(39929979000 ns) = 119789937000 ns; the actual ratio is ≈5.02×, comfortably above the frozen floor even against the worst-case (this time, rep1-arm) latency.
- licence-condition: repetition-variance — met — the two subject repetitions' own overlap margins, 249686021000 ns (`resolved-type-n3-rep1`) and 270647473000 ns (`resolved-type-n3-rep2`), differ by ≈7.7% — a small, undramatic spread, disclosed descriptively; no significance test, confidence interval or third repetition is claimed from it.
...
- **Evidence versioning, history kept rather than smoothed over.** The first probe's evidence (commit `2fa4ccd0a69a8a0bd26cf48fc1f51d902911bed8`) was declared invalid under the frozen invalid-evidence rule for a recorded, non-scientific reason: its evidence section was drafted without this spec's own **AC4** required `- command: ` line grammar inside the evidence section itself — a conformance omission by that evidence's own author, never a defect in the frozen protocol, the differential design, or the direction of the result. A proposed repair that instead narrowed **AC4**'s own check was raised and explicitly rejected by the operator as itself the result-fitting the freeze-then-probe design exists to forbid (`.shell-team/interventions/T-1111.md`, fifth entry). The disposition taken was the one permitted re-probe: the identical frozen protocol, re-run end to end in a fresh clone, evidence drafted to the frozen grammar this time (`## Probe evidence` above, four tagged `- command:` lines). Every analysis figure in this note is derived from that second, valid evidence commit (`074ee2f67208c43416aa66c63f8545fcf74c29ef`) alone; the first probe's raw integers are superseded, not merged, and no figure in this note traces to them.
- No inferential-statistics claim anywhere in this note: the two subject repetitions' ≈7.7% overlap-margin spread is disclosed descriptively (`- licence-condition: repetition-variance` above); no significance test, confidence interval or regression is computed over it, and no dollar cost, vendor-price or token-multiplier estimate appears anywhere in this note.
