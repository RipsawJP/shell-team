...
measurements, on two different corpora, and neither time found retirement
stock that would shrink a generated block. Without a citable place holding
that answer, the warning was a standing invitation to launch a third check of
a question two prior checks had already answered. This document is that
citable place: the warning now points here instead of at a task, and a future
reader can read the five claims below instead of re-deriving them.
...
- This repository's `.shell-team/specs/T-1020-supersede-adjudication.md`
  (issue #97) walked 24 numbered candidates on 2026-08-03: 22 of them were
  scored against a stricter three-condition predicate, and the other 2 were
  excluded from scoring by a machine pre-filter before any successor was even
  considered. Among the 22 scored, exactly one ratifiable pair was found —
  `Scope: maintainer`, so zero block-line impact. Its section "What this accounting
...
- claim-1: The dilution that the 40-line threshold proxies for is a real mechanism, but its harm was unmeasured and unobserved at the pre-publication re-evaluation, and the T-1020 adjudication of 2026-08-03 measured nothing about it either — at neither point was it measured and found to be small.
- claim-2: The value 40 in LINE_WARN_THRESHOLD=40 has no technical basis: it was a judgment call, not a token budget and not a measured degradation point.
- claim-3: Two independent sweeps looked for retirement stock in the corpus and neither found any that would shrink a generated block: the pre-publication exhaustive pairwise domination sweep of 2026-07-26 over all 74 then-active entries found zero dominations, and the T-1020 adjudication of 2026-08-03 in this repository scored 22 of its 24 numbered candidates against a stricter three-condition predicate — the other 2 were excluded from scoring by a machine pre-filter before any successor was considered — and found exactly one ratifiable pair among the 22, which is Scope: maintainer and therefore has zero block-line impact.
- cause: Both sweeps ran into the same reason: the promotion discipline writes positioning sentences at write time, so the corpus does not accumulate the retirement stock a sweep needs to find — and the one ratifiable pair the 2026-08-03 adjudication did find was maintainer-scoped and shrank no block.
...
- signal-3: The corpus grows by an order of magnitude beyond the roughly 80 active entries the 2026-08-03 adjudication covered.
...
- Not a defense of the number 40: claim-2 already states that the threshold
  ...
  stock that would shrink a generated block is not the same thing as supplying
  a basis for the number itself, and this record does not attempt to.
