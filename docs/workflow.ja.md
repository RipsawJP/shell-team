...
機械的に検出可能な 3 つの地点——same-class-2 のカウントが 2 に達した時、
spec pre-commitment の factual trigger が発火した時、loop-guard の
`STOP:` escalation——のそれぞれで、loop はあなたへの hand-off を組み立てる
前に **means-ends reflection**（T-1095、issue #346）を実行する。4 つの
固定質問に書面で答えたうえで、次の 3 条件がすべて成り立つ場合に限り、
その disposition を loop 自身の権限で execute し、4 つの答えを根拠として
board に記録する（安く見える別の patch と並ぶ一選択肢として escalate
するのではない）。3 条件とは、(1) タスク自身の never-dropped 項目が
すべて green のまま、(2) findings が対象とするのが、すでに名前の付いた
auxiliary component（複数ある場合は pre-commitment に記録された drop
...
の場合は、あなたへの escalation が引き続き必須となる。loop の固定層が
保証するものは変わらない: タスクが done になるのは両方の gate が green に
なった時だけであり、loop は自分の判断だけで merge することはない——
merge は人間の行為である。破壊的・不可逆な操作の前の停止を含め、それ以上
の停止が入るかどうかは the operator's own oversight configuration
が追加するものに委ねられる（詳細は
...
- **仕様**: `<specs dir>/T-XXX-<slug>.md`、slug はタイトルの kebab-case、`<specs dir>` は `team-paths.sh --get specs`（`bash "<plugin root>/bin/team-paths.sh"` として起動——プラグインをロードしていても `PATH` に載るとは限らない。`<plugin root>` は自ホストの報告値から読む。詳細は `docs/adopting.md` の "Locate the installed plugin root" 手順を参照）
- **レビュー成果物**: `<reviews dir>/T-XXX.md`（curated verdict + severity ledger）と Codex 生トレース `<reviews dir>/T-XXX-codex-*.{txt,jsonl}`、`<reviews dir>` は `team-paths.sh --get reviews`（上と同じ起動上の注記が当てはまる）
