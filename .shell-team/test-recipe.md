...
- Lint/format commands: no formatter runs; four steps named "Lint the shipped …" run the repository's own linters — `bash bin/check-handoff.sh templates/todo-template.md`, `bash bin/check-contract.sh templates/shell-team.contract.yaml`, `bash bin/check-contract.sh templates/loop-contract-template.yaml`, `bash bin/check-contract.sh templates/goal.contract.yaml` (corrected 2026-09-10 after review round 3: the first fill-in claimed none)
- Static-analysis commands:
  ...
  shellcheck tests/bin-help/run.sh
  ```
...
- T-1134: `bash tests/codex-agents/run.sh` covers `bin/gen-codex-agents.sh` and
  `bin/check-codex-agents.sh` together in one suite (they share every fixture
  — the sync checker is only meaningful against the generator's own output).
  Run it exactly like any other `tests/<suite>/run.sh`; no extra setup. Its
  one non-obvious fixture (the Codex-adapter model/`model_reasoning_effort`
  emitting branch, unreachable under this repo's own shipped default
  binding) supplies a Codex-adapter row through a `$TEAM_RUN_BASE`-resolved
  fixture `binding.conf`, exercised by `cd`-ing into a scratch working
  directory and exporting `TEAM_RUN_BASE` before invoking the generator —
  the same override path an adopter's own custom binding uses, not a new
  mechanism. `bin/gen-codex-agents.sh`'s own executor-binding read always
  goes through its sibling `bin/resolve-executor.sh` next to it, never
  through `--root`, so a fixture `--root` holding only `agents/*.md` (no
  `bin/`) still generates correctly.
...
- T-1116 (harness invocation parameter probe — engineer verification): assembled the note's eight non-frozen sections from the coordinating session's committed `## Probe evidence` (touched zero bytes of it) and this spec's frozen `probe-protocol` region (reproduced verbatim). Verified `## Probe evidence` byte-identity against the working tree with `awk '/^## Probe evidence$/{f=1;next} f&&/^## /{exit} f' <file>` on both the committed blob (`git show <sha>:docs/loop-engineering/harness-invocation-parameters.md`) and the assembled note, then `cmp`. Ran `CHECK_ACS_TIMEOUT=900 bash bin/check-acs.sh .shell-team/specs/T-1116-harness-invocation-parameter-probe.md`; `CHECK_ACS_TIMEOUT` is inert on this host (no `timeout`/`gtimeout` resolves), consistent with the sibling-task finding already recorded above, so the wrapper ran a plain (unbounded) invocation — this took under a minute in practice given the check lines are all pure-bash/grep/awk with no network or long-running step. Ran the nine AC15 gate checkers individually (`check-provenance.sh`, `check-interventions.sh --task T-1116`, `check-count-claims.sh`, `check-entry-mode.sh`, `check-spec-review.sh`, `check-intent.sh`, `check-handoff.sh`, `check-board-headings.sh --base <branch point>`, `check-pii-shapes.sh --base <branch point>`) and recorded each exit code in the hand-off rather than trusting AC15's own aggregate pass/fail alone.
- T-1117 (pin/default equality lock): `bash tests/check-model-pins/run.sh` is `bin/check-model-pins.sh`'s fixture suite (a CI-wired, fail-closed equality lock between every `claude`-provider role's frontmatter `model:` pin and `templates/binding-default.conf`'s model token for that role — makes T-1057's frozen AC5 equality clause continuous; issue #235). No new prerequisite: pure bash + git, same synthetic-fixture-in-a-scratch-dir convention as `tests/check-binding/run.sh` and `tests/resolve-executor/run.sh` — its scratch root is the `${TMPDIR:-/tmp}`-only `mktemp -d ... XXXXXX` arm (no `$HERE` fallback), because several cases copy an installed tree (`bin/` + `templates/` + `agents/`) out of the checkout to mutate a scratch copy, the same restriction the T-1057 entry above documents for the identical reason. Two authoring traps specific to this checker, both load-bearing: **(1)** the population is delegated from `check-binding.sh --print-binding` and re-asserted by content (schema-line shape/version, each `bound` row's field count, role uniqueness) but deliberately does **not** re-assert the row count or role membership — those stay `check-binding.sh`'s `missing-role` refusal and `resolve-executor.sh`'s own exactly-six-rows clause, never duplicated here (DP-3), so a scratch stub sibling used to exercise the canonical-form re-assertion (the `cmp-canonical-reasserted` case) must be built as a full installed-tree copy (`build_installed_tree`), never a bare `--config`-style override, since this checker has no adapter for `check-binding.sh` itself. **(2)** the pin-reading regex is line-anchored `^model:` with **no trailing space required** — a bare `model:` line (nothing after the colon) must still count as one match so it lands in the empty-value arm (exit 2), not the absent-pin arm (exit 1, `#236`'s Branch A shape); a pattern requiring `^model: ` (with the space) would silently misclassify it. Because the checker joins `tests/check-binding/run.sh`'s live `cb-adapters-not-forwarded-population` population (it names `check-binding.sh`), it must carry zero occurrences of `--adapters` anywhere — code, comment, or `--help` header — confirmed via `grep -cF -- '--adapters' bin/check-model-pins.sh` = 0. Measured this run: `bash tests/check-model-pins/run.sh` → 13 `PASS:` lines, 0 `FAIL:` lines (`grep -c '^PASS:' <log>` = 13, `grep -c '^FAIL:' <log>` = 0); `shellcheck bin/check-model-pins.sh tests/check-model-pins/run.sh` → exit 0 (0.11.0); `bash tests/check-binding/run.sh`, `bash tests/errexit-safe/run.sh` and `bash tests/bin-exec-bit/run.sh` (the two mechanically-coupled suites this task's new files join, per DP-9, plus the bin/tests exec-bit lock) all exit 0 with the new files staged at git index mode `100755`.
- T-1118 (alternate-executor invocation path — engineer verification): two authoring traps this task's own reading found, both load-bearing for the next task landing anything in `skills/run/SKILL.md` or amending `templates/prompt-blocks/executor-resolution.md`. **(1) `check-prompt-sync`** — `templates/prompt-blocks/executor-resolution.md` is registered `contain` with **four** consumers (`skills/run/SKILL.md`, `skills/goal/SKILL.md`, `skills/review/SKILL.md`, `skills/review-response/SKILL.md`); amending the canonical file and only one or two consumers leaves `bash bin/check-prompt-sync.sh` red — run it after every edit to a `contain`-mode canonical block, not only after registering a new one. **(2) `codex-skeleton-hygiene`** — `tests/codex-skeleton-hygiene/run.sh` forward-locks `skills/run/SKILL.md` with two frozen regexes (`bash bin/check-(provenance|interventions)\.sh` in any path-prefixed form, and `(provenance|interventions)[-\s]+gate:AC[0-9]|route[-\s]+back[-\s]+through[-\s]+loop[-\s]+guard`) that must match **zero** lines there; a new prompt block appended to that file must avoid both patterns in its own text, checked with a positive-control fixture before trusting a zero-match result over the real file. **A host-tool defect worth inheriting**: a `- check:` line built as `grep -oE '.{0,400}<literal>.{0,400}'` fails outright on macOS's stock `/usr/bin/grep` (BSD grep 2.6.0-FreeBSD) with `grep: maximum repetition exceeds 255` — that grep's interval-matching cap is 255, well under 400, and the failure is a hard compile error rather than a wrong match; the same construct is expected to work under GNU grep (Ubuntu CI runners). A window-co-location check of this shape should keep the interval at or under 255 (or use a length-bounded `sed`/`awk`/`cut` extraction instead of a large-repetition `grep -oE`) if it needs to run identically on both grep families. Measured this run: `bash tests/check-invocation-path/run.sh` → 22 `PASS:` lines (`grep -c '^PASS:' <log>` = 22, one per named case in `## Acceptance criteria`'s AC15), plus one documented, expected `FAIL:` line for `cip-default-all-six-admit` (see this task's hand-off for the underlying spec contradiction it evidences — not a defect in the fixture); `shellcheck bin/check-invocation-path.sh tests/check-invocation-path/run.sh bin/resolve-executor.sh` → exit 0 (0.11.0); `bash tests/codex-skeleton-hygiene/run.sh`, `bash tests/check-prompt-sync/run.sh`, `bash tests/errexit-safe/run.sh`, `bash tests/bin-exec-bit/run.sh`, `bash tests/check-binding/run.sh` and `bash tests/check-adapter/run.sh` all exit 0 with the new files staged at git index mode `100755`.
- T-1119 (effort application — engineer verification): three procedures this task's own reading established, all reusable by the next task that touches an inline CI step, the `templates/prompt-blocks/alternate-executor-invocation.md` recipe, or a Japanese adopter surface. **(1) Extracting and running a workflow's inline `run: |` step from a scratch tree** — the idiom `.github/workflows/check-handoff.yml`'s new `applies-effort` step's own mutation arms use, and the reusable shape for giving any inline CI step mutation coverage without a `tests/` fixture or an override affordance: extract the body with `awk` keyed on the step's own `- name:` literal and its exact indentation (`- name:` at 6 spaces, `run: |` at 8, body at 10 — dedent by taking `substr($0,11)`), copy `templates/` into a scratch `mktemp -d` tree, `cd` into it and run the extracted body with plain `bash`. **The extracted body needs its own `set -e`**: without it, a bare `test X = Y` failing partway through a multi-line script does not abort the script (no `set -e`, no explicit `exit`), so the script's own final exit status is whatever the *last* command happened to return — a fail-closed-looking script that is silently not fail-closed at all. Verify this by mutating a target file the extracted body reads and confirming the pristine-vs-mutated arms actually diverge, not just that the script parses. **(2) The scratch installed-tree resolver arms (AC7's idiom, reused by this task's own protocol step 0)** — `bin/` plus `templates/` copied out of the checkout, a host `binding.conf` written under the copy's own `.shell-team/`, then `cd` into the copy with **no environment variable set at all** (a `cd`, not a `TEAM_RUN_BASE` override — `bin/team-paths.sh` refuses an absolute `TEAM_RUN_BASE` outright, per T-1116/T-1119's own DP-i measurement) and `bash bin/resolve-executor.sh --print-resolved` run from inside. **(3) `check-prompt-sync` after every edit to a `contain`-mode block, not only after registering a new one** — same discipline T-1118 already recorded, reconfirmed here: amending `templates/prompt-blocks/alternate-executor-invocation.md` without mirroring the same edit into `skills/run/SKILL.md` in the same commit leaves `bash bin/check-prompt-sync.sh` red. **A pre-existing `contain`-mirror structural trap, found and repaired here rather than disclosed**: `check-prompt-sync.sh`'s own `contain` mode matches each canonical line as a fixed-string **substring** in the consumer (`grep -qF`), which is why a shipped consumer paragraph can carry a role-specific tail appended to the *same physical line* as the block's own last sentence (`skills/run/SKILL.md`'s "Apply this at step 1's `tech-lead` invocation…" tail on the alternate-executor-invocation block's closing paragraph, mirroring the pattern the adjacent Executor-resolution block's own tail already used). A stricter, **whole-line** equality check (`grep -vxFf`) over the same "every non-empty canonical line must survive" property does not tolerate that shape and reddens on a line this task never touched — repaired by splitting the tail onto its own paragraph (meaning-preserving; no spec was found to pin an absolute line number in `skills/run/SKILL.md`, confirmed by `grep -rn "sed -n '[0-9]*p' skills/run/SKILL.md" .shell-team/specs/*.md` returning nothing, so a reflow here does not silently break another task's frozen criterion). **A grep-vs-UTF-8 trap specific to Japanese adopter surfaces, worth inheriting for any future `awk substr()`-cut window over CJK text**: a check that cuts a fixed *byte* window out of a UTF-8 file with `awk`'s `substr()` under `LC_ALL=C` can truncate a multi-byte character mid-sequence at either boundary; the resulting incomplete byte sequence makes BSD `grep` (without `-a`) classify the extracted window as binary and refuse to match **anything** in it — including a plain ASCII literal that is visibly present — so every `grep -qF` against that window spuriously reports absence, with no error message distinguishing it from a genuine miss. Diagnose with `file <window>` (`Non-ISO extended-ASCII text`, not `ASCII text`) or a `python3 -c "open(...,'rb').read().decode('utf-8')"` probe; `grep -a` confirms the content is actually present. The fix is not a `-a` flag (the frozen check line does not carry one and is not editable) but padding the source text with a few extra ASCII bytes *after* the anchor and *before* the window's own far boundary, chosen empirically by trying short ASCII insertions of increasing length until the window decodes cleanly — inserting bytes *before* the anchor does not help, because it only re-addresses the same relative content rather than adding new bytes to what the window can reach. Measured this run: `CHECK_ACS_TIMEOUT=900 bash bin/check-acs.sh .shell-team/specs/T-1119-effort-application.md` → 23 passed, 0 failed, 1 skipped (`grep -c '^AC[0-9]*: PASS' <log>` = 23); `bash bin/check-prompt-sync.sh`, `bash bin/check-adapter.sh`, `bash bin/resolve-executor.sh --print-resolved`, `bash bin/check-invocation-path.sh --role tech-lead` and `bash bin/check-readme-version.sh README.md README.ja.md` all exit 0; `shellcheck --shell=bash` (0.11.0) clean on the new inline workflow step's extracted body; the joined suites `tests/{check-adapter,resolve-executor,check-invocation-path,check-binding,check-prompt-sync,check-model-pins,codex-skeleton-hygiene,errexit-safe,bin-exec-bit}/run.sh` all exit 0; a byte-identical before/after `bash bin/check-acs.sh` diff over T-1060, T-1105, T-1112, T-1113, T-1114 (the JA-mirror doc-literal and predecessor byte-lock classes this task's own diff reaches) confirmed zero regressions beyond the disclosed, predicted reds.
- T-1140 (tracker-key and host-local PII shapes — engineer verification): two environment quirks worth inheriting. **(1) A `"$VAR:literalword"` double-quoted string, run through this checkout's Bash tool, silently drops the colon and the literal word's leading character** — `Y="abc"; echo "$Y:tests/foo"` prints `abcests/foo`, not `abc:tests/foo` (reproduced live, this session); `"${Y}:tests/foo"` (braced) and `"$Y:$Z"` (variable-then-variable, the shape every shipped `- check:` line in this repo already uses, e.g. `git show "$B:$S"`) are both unaffected. This is a preprocessing quirk of the tool layer, not of bash itself or of any shipped check line — but it bit an ad hoc `git show "$B:tests/..."` verification command during this task and would bite the same shape again; brace the variable or use a second variable when constructing a `ref:path` argument by hand. **(2) `bin/derive-populations.sh`'s `--set NAME=<pipeline>` legitimately produces an empty result once a corpus token this task's own implementation resolves is gone** — a `git grep ... | sort -u` pipeline with zero matches exits 1 under the tool's own `pipefail` semantics, which is a genuine, expected outcome for a set that is supposed to be empty after a fix lands, not a usage error; pass `--accept-status <name>=1` for that set (the tool's own `--help` names this exact case) rather than treating the refusal as a bug. Measured this run: `bash tests/check-pii-shapes/run.sh` → 114 `PASS:` lines, 0 `FAIL:` lines (`grep -c '^PASS:' <log>` = 114, `grep -c '^FAIL:' <log>` = 0); `shellcheck bin/check-pii-shapes.sh` → exit 0 (0.11.0), with `# shellcheck disable=SC2034` needed on the tracker-key outline variable (read only by the fixture suite's own precondition idiom via `grep`, never referenced inside the script itself); `bin/check-pii-shapes.sh --all` and `--base develop` both exit 0 once the two authoring-site token repairs land. A pre-existing, unrelated false-positive population in AC11's own residual-grep filter (`window.localStorage` in `templates/loop-replay.html`, `mail.localhost` in this suite's own reserved-domain fixture) is disclosed in this task's spec under `## Notes from engineer` rather than repaired — see that section before assuming a red AC11 means this task's own diff leaked a shape.
- T-1151 (adopter-docs checker rebuild + issue #577 shipped-docs inventory — engineer verification): `bash tests/check-adopter-docs/run.sh` is `bin/check-adopter-docs.sh`'s fixture suite (pure bash + git, synthetic-fixture-in-a-scratch-dir convention, `${TMPDIR:-/tmp}`-only `mktemp -d ... XXXXXX` root with the same-directory fallback tests/check-refreeze-class/run.sh already uses). The checker is a stateless three-variable (F x R x S) state-machine predicate over one spec file — see the checker's own header comment for the transition table; the suite's case inventory is derived one-way from that table (documented at the top of the suite file). Two authoring traps specific to this checker: **(1)** `R` (the declaration region) closes at the FIRST unfenced `^## ` heading after `BEGIN`, not specifically `## Non-goals` — a spec whose intent block carries no `## Non-goals` heading at all still closes `R` at whatever heading comes first (`cad-region-after-first-heading` / `cad-region-before-first-heading`), a deliberate departure from the carved-out generation's `## Non-goals`-specific fallback. **(2)** `shipped-docs-unmeasured` is decided as a pure string relation between lines of the same file (`^[[:space:]]*- check:` / `^[[:space:]]*- adopter-surface:`, path tested as a literal substring) — never a filesystem read; `cad-nofs-exists-unmeasured` / `cad-nofs-absent-measured` invert filesystem truth against the checker's verdict to prove this (an on-disk-existing, unmeasured path is still refused; an on-disk-absent, measured path still passes). **(3, round 2 — QA round 1's reading-judgment-sweep finding)** a FENCED `^## ` heading sitting INSIDE `R` (before `R` closes) must not close it — `cad-region-fence-before-decl-pass` / `-control` is the discriminating fenced/unfenced pair; the symmetric placement *after* the declaration (`cad-region-fence-after-decl-pass` / `cad-region-plain-after-decl-still-pass`) is a **non-discriminating** confirmation, reported as such rather than mislabelled as a red/green control — a declaration is classified by its own line's `R` value at match time, so a heading appearing after it never retroactively changes that verdict, fenced or not (verified live before writing the fixture). **(4, round 2)** the `issue #<N>` disposition grammar's own edge cases (`issue #abc`, `issue #`, `issue #12a` all `shipped-docs-malformed`; `issue #0` passes — the grammar requires digits, not a positive integer) needed their own fixtures (`cad-shipped-issue-*`), not just the one `issue #577` happy-path case. **(5, round 3 — cross-provider review Major, class `positional-boundary-unbounded-at-END`)** `R` alone has no upper bound at the intent block's own `END` marker: a block carrying no `^## ` heading before `END` left `R` open past `END`, so a well-formed `- user-visible:`/`- shipped-docs:` line sitting in that dead zone (after `END`, before the file's next real heading) was recorded well-placed. Fixed by requiring the same `i < end_ln` clause `WAIVER_RE`/`SURFACE_RE` already carried, added as a second positional clause alongside `R == 1` (R's own closing rule — first `^## ` heading — is unchanged) at the two decision sites (`bin/check-adopter-docs.sh`'s declaration and shipped-docs misplaced checks); mutation-tested by reverting each of the two `-ge "$end_ln"` clauses independently in a disposable clone and confirming `cad-region-end-bounds-decl`/`-shipped` each catch it. `cad-region-end-bounds-decl`/`-shipped` are the discriminating pairs (bad: line after `END` in a heading-less block; control: identical line inside the block), `cad-region-end-bounds-decl-fenced` confirms a fenced decl-shaped line in the same dead zone stays invisible (F's delimiter classification is unaffected by the new bound). `MEASURE_LINES` (the `shipped-docs-unmeasured` support scan) was re-swept and confirmed deliberately whole-file, per AC11 and the Goal's own wording — left unbounded. Measured this run: `bash tests/check-adopter-docs/run.sh` → 75 `PASS:` lines, 0 `FAIL:` lines (`grep -c '^PASS: ' <log>` = 75, `grep -c '^FAIL: ' <log>` = 0); `shellcheck bin/check-adopter-docs.sh tests/check-adopter-docs/run.sh` → exit 0 (0.11.0); `bash bin/check-prompt-sync.sh`, `bash tests/bin-exec-bit/run.sh` and `bash tests/errexit-safe/run.sh` all exit 0 with the new files staged at git index mode `100755`.
- T-1152 (codex sandbox hotfix — every-part matching, rollout import — engineer verification): every criterion and fixture that exercises `bin/codex-capture.sh --publish --thread-id` points `CODEX_HOME` (or, for the one case that must fall back to the default, `HOME` with `CODEX_HOME` unset) at a fresh `mktemp -d` scratch directory carrying its own synthetic `sessions/<YYYY>/<MM>/<DD>/rollout-<timestamp>-<id>.jsonl` fixture — never the operator's real `~/.codex` session store. A minimal synthetic rollout is three lines: one `session_meta` line, one or more `event_msg` lines whose `payload.type` is `item_completed` and whose `payload.item.type` is `CommandExecution` (with an `"exit_code":0` boundary match, not a bare substring, so `"exit_code":01` or `"exit_code":0.5` never counts as success), and a terminal `event_msg` line whose `payload.type` is `task_complete` — only the LAST such line in the file counts when more than one is present. `--thread-id` requires the id to match `^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$` and the caller's own `<raw-jsonl>` to still be the empty file `--alloc` created (the import fills it, never the caller); the gate refuses (exit 3, `review did not run`) a rollout showing zero succeeded commands, no `task_complete` event, a null final message, or a non-null error, and reports (never gates on) a `failure_markers` count over `turn_aborted|workspace routing discovery failed|sandbox_apply|turn\.failed`. `--alloc` and every `--publish` call without `--thread-id` are asserted byte-for-byte unchanged against the branch-point script (`git show 30d71fcb:bin/codex-capture.sh`) over five fixtures (ok/sandboxed-but-published/empty-out/bad-jsonl/canon-as-dir), which is also the proof that no token scan was added to that branch.
