...
**The two authoring-site resolutions this task performs.** The `all` set's command ends in a negative lookahead, so it emits only tokens with **nothing** after the machine-local suffix — exactly the shape the `host-local` rule reports. At the branch point (`## Assumptions` records the branch-point command and the expectation of a non-empty result there), this set held one such token, occurring in two tracked records where a configuration file was named without its extension. Those two occurrences were this task's whole corpus work: each is resolved at its authoring site by appending the extension, never by widening the rule and never by list growth, which is why **AC15**'s allow-list names those two paths and `## Notes for engineer` names the edit. The block below is re-derived after both resolutions land, so the `all` bucket is empty and this spec carries no bare token of its own; `git grep`'s own exit 1 for "no match" is the expected outcome for a genuinely empty set and is declared with `--accept-status all=1` rather than worked around. **AC11** is the instrument that would have caught an unresolved occurrence.

- reproduce: bash bin/derive-populations.sh --label T-1140-host-local-lookalikes --set "all=git grep -o -h -i -P '\b[A-Za-z0-9-]+\.local(?![.A-Za-z0-9-])' -- . | sort -u" --set "config-file=git grep -o -h -i -P '\b[A-Za-z0-9-]+\.local\.(md|json|yaml|yml|conf|toml)\b' -- . | sort -u" --set "placeholder=git grep -o -h -P '<host>\.local' -- . | sort -u" --accept-status all=1

<!-- BEGIN derivation: T-1140-host-local-lookalikes -->
- derived-by: bin/derive-populations.sh
- locale: LC_ALL=C
- set: all — status: 1 — lines: 0 — items: 0 — command: git grep -o -h -i -P '\b[A-Za-z0-9-]+\.local(?![.A-Za-z0-9-])' -- . | sort -u
...
**AC11's own residual-grep filter carries a pre-existing false-positive population, present at the branch point, unrelated to this task's shape.** AC11's check line gathers candidate lines with the broad, unanchored `[A-Za-z0-9-]\.local` and exempts a candidate only when the SAME line also contains the literal substring `.local.` (a dot-led continuation, read as "this is a file extension"). That exemption does not cover a continuation that is *more letters with no intervening dot* — `.localhost` and `.localStorage` among them — which is exactly the boundary the real, shipped `RE_HOST_LOCAL` already excludes correctly (its right-side requirement is end-of-line or a character that is not a letter, digit, or dot; a further letter fails that requirement precisely as a dot-led extension does), but which AC11's own cruder filter does not recognise. Measured at the branch point (`B=$(git merge-base develop HEAD)`; `git grep -nE -e '[A-Za-z0-9-]\.local' "$B" -- 'bin' 'tests' 'docs' 'agents' 'skills' 'templates' '.shell-team' '*.md' | grep -vE '\.local\.'`): four residual lines. One is the disclosed cause this task resolves at `.shell-team/interventions/T-1109.md:25` (the unextended configuration-file base name; the second record this task also fixes, `.shell-team/reviews/T-1110.md`, does not appear separately in this line-level count at all — its own single line already contains its own `CLAUDE.local.md` mention with the extension, so AC11's line-level substring filter exempts that whole line regardless of the bare mention sharing it, which is itself a further sign of this filter's coarseness; the shipped, per-token checker still reported that file correctly before the fix, and reports it correctly resolved after). The other three predate this task entirely and are unrelated to the host-local shape: `templates/loop-replay.html:542` and `:1669` (a `window.localStorage` JavaScript API call, present at the branch point; AC11's own broad, unanchored pattern matches the single letter immediately before the dot in that property name plus the word "local" it starts with, with nothing after it to signal "this is a file extension" — not a host name, not this task's own content) and `tests/check-pii-shapes/run.sh:581` (this suite's own pre-existing `mail.localhost` reserved-domain email fixture, likewise present at the branch point and unrelated to host-local). All three of the latter are out of this task's `## Notes for engineer` allow-list and out of scope under Non-goals ("Only the sites the extended checker actually reports in its shipped default mode are in scope") — `bin/check-pii-shapes.sh --all` on this tree, the actual shipped checker, does not report any of them as `host-local` (verified live), so no real leak exists; the gap is in AC11's own verification-only filter, not in the shipped detection logic. Disclosed here rather than repaired: the check line sits inside this spec's frozen intent block, and Non-goals forbids widening or repairing a merged criterion's own text; this is that same discipline applied to this task's own criterion rather than to another task's. Recommendation for QA and the reviewer: treat AC11 as carrying this disclosed, pre-existing residual of 3 lines once this task's own record fix lands (confirmed unrelated to this task's diff by the branch-point measurement above), and confirm the substantive property AC11 exists to protect — no byte of the new host-local shape entering the tree — through `bin/check-pii-shapes.sh --all`'s own clean exit instead.
