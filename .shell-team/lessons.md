...
- **Status**: active
- **Source**: n/a
- **Rule**: When the same class of norm or token appears across more than one canonical file (an operative doc and a governing spec, plus a board), don't fix just one — before starting, do a repository-wide grep inventory of every file and every occurrence, and fix them all in one round. A canonical pair is not counted by file boundary alone — prose within a single file that states a discipline, and the concrete code or fenced block that discipline governs, also count as a canonical pair that belongs in the same inventory. The inventory's scope is the whole task diff plus every stream that diff produces for an adopter (a script's `--help` text, its stderr messages, a suite's labels) — never a subset of directories or files named by whoever writes the rework instruction — and when the defect is one cell of a small combinatorial table (a role-scope × occupant-type 2×2, for instance), the instruction enumerates every cell of that table by name, not the reported cell plus its neighbour.
- **Why**: A rework round fixed a `.gitignore` contradiction in an operational checklist alone, while the identical misstatement was left behind in six other spots across the governing spec (its input space, several acceptance criteria, a design-decision section, and its notes), and resurfaced as a same-class Major finding in the very next round. An up-front, cross-file inventory of the checklist plus the spec together would have prevented that extra round. Two later tasks in one release repeated the shape from the other side: a rework instruction that scoped the class inventory to `docs/` and one skill line let the same forbidden sentence survive in the script's own `--help` header, and a rework instruction that asked for two variants of a remedy-claim defect left the fourth cell of the 2×2 open — each cost a same-class-2 escalation and an operator GO (retro `.shell-team/retros/2026-09-17-codex-upgrade-b-run6.md`).
- **How to apply**: At the start of a rework, once a norm or token is known to appear in more than one canonical file (a checklist, a spec, the board), grep across every canonical file for it, list every occurrence, and fix them in one pass. pm-spec should state explicitly in a rework instruction that the target is every file in the diff's scope, and the engineer should self-inventory before implementing. This extends the existing same-class-2/mirrored-application discipline across file boundaries. When prose changes a discipline, the same hand-off must also update any code block or fence governed by that discipline — detecting a half-updated change needs a structural lock on the code side, not just a presence lock on the prose. The rework instruction states the inventory scope as the whole diff plus its adopter-visible streams in one sentence, and for a table-shaped defect lists every cell; the engineer reports the inventory (hit, verdict, before/after) in the hand-off so QA can audit coverage rather than re-derive it.
...
- **Extended by**: 2026-09-08 — the variant list includes sibling sites that copied the same precedent: when the defective shape was written by copying an existing family (a stem regex, a parser arm, a lock line), every other family built from that precedent carries the same class and is enumerated with the variants; a sibling the task's frozen Non-goals exclude is filed as a fast-follow naming the class, never left unmentioned. Measured case: a colon-anchored stem copied from an earlier board family let a colon-less spelling read as the conformant absent case; the review closed it for the new family, and the two sibling families that had lent the shape still carried the class until the close-out filed them.
...
- **Extended by**: 2026-09-07 — the declaration moves one step earlier for `entry-mode: operator-authored` specs: at freeze, pm-spec writes one line naming who ratifies a class-B re-freeze (the operator in person, or a named authoring session acting for them), so the orchestrator that later hits `drift-detected` reads the ratifier from the spec instead of deciding the procedure itself. Measured case: an operator-authored spec relayed from an authoring session to a run session reached class-B re-freeze with no ratifier declared; the run session invented a 'the operator must type into this session' procedure, negotiated it with the authoring session, and drew two operator corrections plus one blocking question that the one line would have removed.
...
- **Extended by**: 2026-09-07 — the per-item table gains a third check beyond the two tier tests: whether the record grammar and check syntax the item must write into (the derivation record's required fields and closed verdict set, the board format) already express the state the item asserts. An item that reads as a one-sentence adopter-doc or skill correction but needs a value no existing field or enum carries is a gate-design item, sized and scheduled as one, not a wording fix. Measured case: an item planned as a one-sentence documentation correction turned out to require a verdict value for "premise present, rule absent" that the derivation grammar had no way to record; the reviewer rejected two consecutive wording attempts, the estimate lapsed the same day the backlog was approved, and the sprint closed two of its three goals.
...
## 2026-09-07 — A proposal to drop or defer an item after its estimate lapses grounds itself in the measured failure, the alternative approach and why later — never in a rule or policy name
- **Category**: process
- **Applies-to**: tech-lead, pm-spec
- **Scope**: loop
- **Status**: active
- **Source**: .shell-team/retros/2026-09-07.md
- **Rule**: When an item's estimate lapses (the same component draws consecutive REQUEST_CHANGES or FAIL verdicts) and the proposal is to remove it from the running cycle, the proposal states three things — the measured failure as concrete fact (which component, how many consecutive rounds, which verdicts), the alternative approach that would close the item, and why that approach belongs to a later cycle rather than this one — together with the goal count the cycle reaches without it (N of M); a rule, policy or lesson name is context at most and is never the ground for the drop.
- **Why**: A rule citation skips the self-check a substantive ground forces — whether narrowing approval has become the purpose, or whether the deferral protects a visible metric instead of the goal — so a drop justified by a policy name had to be re-justified in a follow-up exchange with the operator before it could stand; the substantive form also hands the next planning the failure measurement and the alternative it needs, which the citation form does not carry.
- **How to apply**: Before sending an escalation that removes an item from the running cycle, check that the text names the measured failure, the alternative approach, the later-not-now reason and the resulting goal count; delete any sentence whose only ground is a rule, policy or lesson name and replace it with the fact that rule was written for.

## 2026-09-07 — A briefing or board edit that writes a line a checker or a role boundary validates copies the current precedent or definition instead of recalling its shape
- **Category**: verification-discipline
- **Applies-to**: all
- **Scope**: loop
- **Status**: active
- **Source**: .shell-team/retros/2026-09-07.md
- **Rule**: Before writing a record line that a checker validates (a dispatch line, a version-derivation line, a hand-off block) or naming a write target that a role's boundary governs, the author opens the current precedent — the most recent accepted line of that kind on the board, or the addressed role's definition and its write-boundary section — and copies the shape from that source; a line written from memory is not written, however confident the author is of the form.
- **Why**: Two symptoms of one generator appeared in consecutive sprints: a briefing named a write target the addressed role is forbidden to touch, and a briefing wrote a machine-checked record line in a remembered shape that the close-out checker rejected twice, each rejection costing a round between the coordinating seat and the implementing roles; in both cases the correct form was one file away and the failure was recalling instead of reading. The checker catches the malformed line, so the cost is rounds rather than corruption — which is why the remedy is prose at the authoring seat, not another checker.
- **How to apply**: When composing a briefing, a board entry or any line that a script in bin/ later validates, find the latest accepted line of the same kind on the board and copy its shape before filling in the values; when naming a file or flag a role should write, read that role's definition for the permission first; treat a checker rejection of a record line as the signal that this step was skipped, and return to the precedent rather than patching from memory.
- **Extended by**: 2026-09-10 — the same generator has a third symptom, caught by the cross-provider review rather than by a checker: a briefing that paraphrases the frozen Goal instead of quoting it. The coordinating seat restated a Goal clause that named a section ("names the section") as an obligation to execute it ("fills in and runs"), the engineer implemented the stronger wording, and the reviewer returned it as a Major against the frozen scope, costing one rework round. When a briefing carries a scoped obligation from a frozen spec to an implementing role, the author copies the Goal's Body-to-AC wording verbatim and adds no obligation vocabulary of its own; a briefing sentence that cannot be traced to a quoted Goal or AC line is not written. Source: `.shell-team/retros/2026-09-10-ci-parity.md`, `.shell-team/reviews/T-1133.md` round 1 Major 2.
- **Extended by**: 2026-09-11 — the same generator surfaced twice more in one task, on two different kinds of source material: a rework briefing dictated a QA-verdict clause asserting what "every measured live run here did" as fact, composed from memory rather than quoted from the board record it claimed to summarize; and a separate rework briefing collapsed the prior review round's per-item fast-follow dispositions into one recollected label, "every fast-follow as untouched," which dropped an item that same review record had already ruled "owed within this task." Both are the recall-instead-of-copy failure applied to a multi-line record rather than a single line: the source to open was a verdict paragraph or a disposition table, not one board line, and the author summarized its shape from memory instead of reading each entry in it. Before writing either kind of sentence, the author lists every disposition, or every clause the sentence will assert as fact, individually from the record first, and composes the sentence only from that list. Source: `.shell-team/interventions/T-1134.md` entries 5-6 (the memory-authored QA clause); `.shell-team/reviews/T-1134.md` round 4 and board round 8's hand-off, which names the dropped item "an orchestration-side omission, not an engineer defect" (the disposition-bundling failure).

## 2026-09-07 — A QA verdict on a criterion that asserts writes to several fields or collections enumerates the target matrix from the spec itself and diffs it against the written payload; the implementation's own test expectations are not the oracle
- **Category**: verification-discipline
- **Applies-to**: qa-verifier
- **Scope**: loop
- **Status**: active
- **Source**: .shell-team/retros/2026-09-07-b-first-run.md
- **Rule**: When an acceptance criterion asserts that values reach a set of write targets — several fields, several collections, several files — and the implementation already carries a passing test that asserts on some of those writes, QA derives the full target matrix (field × target) from the spec's own wording before reading any test, then compares that matrix against the key set the implementation actually writes; a green engineer test proves the targets the engineer thought of, never the set the spec named, so agreement with it is not a PASS ground for the criterion.
- **Why**: In an adopting project's first live run, QA accepted the engineer's assertion of the written record as the correctness baseline for four consecutive rounds while the spec's write-target list included five fields that assertion never mentioned; the omission surfaced only in the third cross-provider review round, the same last-line-of-defense pattern this corpus records for count claims, here for set membership. The rule is prose rather than a checker because extracting write targets from a spec is domain-specific to each adopter's data model; the QA role is the seat that reads both spec and payload.
- **How to apply**: Before running or reading tests for such a criterion, list every (field, target) pair the spec names as a table in the QA report, list the pairs the implementation's write path produces (from the code or a captured payload, not from the test file), and report the set difference; an empty difference is the PASS ground, and any pair present in the spec and absent from the payload is a FAIL named per pair, whatever the test suite reports.
- **Extended by**: 2026-09-08 — the same independence applies to a grammar's negative space: for any criterion whose check parses a line against literal anchors (a colon, a dash, a fixed separator), QA's probe set includes the absence of each anchor and a substitute character in its place, not only the spelling variants the spec enumerates — a spelling that means the family but drops the anchor must be collected and refused, and reading as absent is the defect. Measured case: QA passed all seven criteria from the spec's own eleven enumerated rejections while the cross-provider review's first probe outside that list, a colon-less spelling, read as absent and was a Major.

## 2026-09-11 — A max-iterations STOP escalation reports the round-over-round finding trend and never-dropped-component state, not only the bare iteration count
- **Category**: process
- **Applies-to**: all
- **Scope**: loop
- **Status**: active
- **Source**: .shell-team/retros/2026-09-11-codex-host.md
- **Rule**: When loop-guard's `STOP:max_iterations_reached` fires and the coordinating session writes the extension escalation, the report states, beyond the bare iteration number, the round-over-round count of new independent findings in each lane (QA, cross-provider review), the current state of every never-dropped pre-commitment component, and whether a class label repeated across rounds names the same recurring instance or a fresh distinct one each time — the iteration count alone cannot tell a loop stuck re-finding one defect apart from a loop converging through a sequence of small, non-recurring findings.
- **Why**: Measured in T-1134's nine QA rounds and five cross-provider review rounds: the QA lane's new-independent-finding count per round ran 1,1,1,0,0 and the review lane's ran 3,3,1,1,0, both trending to zero across the task; the operator granted five extensions (iterations 3, 4, 5, 7, 8), but the board's own `extension-granted` lines carried an evaluated (a)/(b)/(c)/(d) option table only at iterations 5, 7 and 8, not at 3 or 4; and the class label `docs-missing-adopter-procedure`, which the cross-provider review cited in four consecutive rounds, turned out on own read to name four distinct instances rather than one recurring defect — a distinction the label alone did not carry and the retro recovered only by reading each finding. Mechanizing this into a checked self-reflection step ahead of a STOP write-up is tracked as issue #491; this entry is the prose precedent for that mechanism.
- **How to apply**: Before presenting a `STOP:max_iterations_reached` extension choice to the operator, derive and include the three items above (per-round new-finding trend by lane, never-dropped-component state, same-instance-vs-distinct-instance reading of any repeated class label) from the board and review record; a report that states only the iteration number has skipped this step.

## 2026-09-11 — A quiet board with no completion notification does not license launching a second role instance on the same worktree
- **Category**: process
- **Applies-to**: all
- **Scope**: loop
- **Status**: active
- **Source**: .shell-team/retros/2026-09-11-codex-host.md
- **Rule**: When a dispatched role's completion notification has not arrived and the board stays quiet for an extended interval — even when an external event, such as a network disconnection, plausibly explains the silence — that silence is not evidence the role has stopped; the coordinating session does not launch a second instance of the same role on the same worktree while neither a completion notification nor an active-liveness-check alarm for the first instance has fired.
- **Why**: In T-1134, a network-disconnection warning was followed by sixty minutes with no verdict on the board and no completion notification from the QA round-2 agent; the coordinating session inferred the agent had died and launched a second QA round-2 instance on the same worktree. The first instance was alive and returned its verdict roughly ten minutes later, so no harm resulted this time, but the task's own intervention record already cites the needed rule as standing local practice — "a completion notification, never a quiet board or a passed interval, is the only signal that a role has finished" — and this recurrence shows that rule living only in this repository's local operating notes was not enough to prevent it; a `grep` of the shipped corpus found no equivalent entry here before this one.
- **How to apply**: Before launching any role, confirm no instance of that role is already dispatched and unconfirmed-complete on the same worktree; on a long silent interval, verify the prior instance's liveness (a completion notification or an active-liveness-check alarm) before dispatching a replacement, and when genuinely uncertain, wait or inspect the worktree/process state directly rather than double-launch.

## 2026-09-11 — A QA verdict whose method is executing a runbook literally names which of the runbook's own state-space branches the run took and which it skipped
- **Category**: verification-discipline
- **Applies-to**: qa-verifier
- **Scope**: loop
- **Status**: active
- **Source**: .shell-team/retros/2026-09-11-codex-host.md
- **Rule**: When a QA verdict's verification method is to execute an adopter-facing runbook or procedure literally against a fixture, the verdict states which of the runbook's own internal state-space branches — fresh install vs. stale install, trusted vs. untrusted repository, and similar forks the procedure itself contains — the executed run actually took, and which branches it did not; executing a runbook literally proves only the branch that particular run walked, never the branches it never reached.
- **Why**: In T-1134 round 6, QA's verdict recorded a literal fresh-fixture execution of the adopter runbook's eight steps and reported no new defect, but that run only ever walked the fresh-install branch. A stale-install branch the same runbook covers went unexercised by round 6, and the cross-provider review's round-3 adversarial pass independently reproduced a missing fallback on exactly that branch — a defect round 6's execution could not have found because its run never took that fork. This is adjacent to, and does not supersede, the 2026-08-02 entry on verifying runnable commands across an execution-context matrix: that entry's axis is the command's external execution context (cwd, PATH, checkout root vs. adopter repository); this entry's axis is the internal state-space branches inside the procedure itself, selected by the fixture's own starting state rather than by where the command is run from.
- **How to apply**: When writing a QA verdict whose verification method is "executed the runbook/procedure literally," first enumerate the branches the procedure's own steps contain, record in the verdict which branch the fixture's starting state put the run on, and treat any branch not walked as unverified rather than covered; a branch is marked green only after a run whose starting state actually selects it.

## 2026-09-12 — An implementation-authored CLI invocation line is run once, exactly as shipped, before hand-off, never inferred from --help text or a syntactically adjacent CLI
- **Category**: verification-discipline
- **Applies-to**: engineer, qa-verifier
- **Scope**: loop
- **Status**: active
- **Source**: .shell-team/retros/2026-09-12-codex-both-gates.md
- **Rule**: When an implementing role assembles a new external-CLI invocation line that will run as-is in an adopter's environment or as another role's dispatch step (a documented recipe block, a generated dispatch line), that exact line — with its exact flags — is executed once against the real CLI before hand-off; the flags' apparent meaning from `--help` text, or analogy to a different CLI's own similar-looking syntax, does not establish their actual semantics.
- **Why**: In T-1135, the shipped `claude -p` recipe was drafted by inferring `--tools`'s semantics from its `--help` text and by analogy to Codex's own permission-rule syntax; on that inference, `--tools "Bash(git diff:*)"` was assumed to grant a scoped Bash tool and `--tools` was assumed to exclude MCP servers. Neither held: the resulting pass carried no Bash tool at all (so `git diff` could not run) and left every `claude.ai` connector tool exposed, including write-capable GitHub/Gmail/Slack/Jira tools. The orchestrator's own live pre-hand-off probe against the real Claude Code CLI (2.1.268) contradicted both premises directly, and a further probe found `--allowedTools` alone does not deny non-listed Bash commands under the adopter's own permission settings either; only the measured, corrected invocation shape was handed to the engineer as fact.
- **How to apply**: Before a spec, an adopter-facing doc, or a generator hands off a new external-CLI invocation line, run that literal line — same binary, same flags — once against the real CLI in a representative environment, and record what it actually admits or denies (tools exposed, commands allowed/denied, servers reachable); that measurement, not the flags' apparent meaning or a resembling CLI's own grammar, is what travels to the next role or into the shipped doc.

## 2026-09-12 — A sandboxed CLI's own seatbelt cannot nest under a wrapped invocation, so a live probe of it calls the CLI's own name as the literal first token
- **Category**: sandbox-constraints
- **Applies-to**: all
- **Scope**: loop
- **Status**: active
- **Source**: .shell-team/retros/2026-09-12-codex-both-gates.md
- **Rule**: When a role invokes a sandboxed external CLI (one whose own sandbox-exclusion mechanism matches on the invocation line's literal first token) as a live probe, it calls that CLI's binary name as the first token of the command, never through a wrapper script or `bash -c`; a sandbox-apply-style refusal produced under a wrapped call is treated first as an artifact of the invocation shape, not as a defect in the target CLI or host, and is re-tested with a bare first-token call on the same fixture before it is written up either way.
- **Why**: In T-1135, QA round 1 recorded `--sandbox workspace-write` and `--sandbox read-only` as "wholly non-functional on this host" (`sandbox_apply: Operation not permitted`, reproduced 4x) and fell back to `danger-full-access`, leaving two runbook state-space rows unverified or substituted. Minutes later, the orchestrator's own bare-first-token `codex exec --sandbox workspace-write` against the same fixture ran cleanly — zero `sandbox_apply` events — and the same mode had already passed four P0 probes hours earlier. The refusal traced to the invocation shape: a `codex` call whose first token is not `codex` runs inside the calling agent's own sandbox, where Codex's seatbelt cannot nest, a pitfall this repository already documents in `bin/codex-capture.sh`'s own header and in `docs/distribution.md` — but scoped there to that script's own design, not stated as a general rule for any role building a live probe, so it did not reach QA before the host CLI was misattributed.
- **How to apply**: Before invoking a sandboxed CLI as a live probe, write the command so the target CLI's own binary name is the literal first token — no wrapper script, no `bash -c`, no intervening shell function; if such a call reports a sandbox-apply-style denial, re-run it as a bare first-token invocation against the same fixture before attributing the denial to the target CLI or host, and record in the verdict which form was actually used.

## 2026-09-12 — A rework that closes a missing-explanation finding by adding new factual prose verifies that prose's own claim against the failure mode it now describes
- **Category**: verification-discipline
- **Applies-to**: engineer, qa-verifier
- **Scope**: loop
- **Status**: active
- **Source**: .shell-team/retros/2026-09-12-codex-both-gates.md
- **Rule**: When a rework closes a review finding that named an absent explanation (a missing prerequisite, a missing branch, a missing caveat) by writing new factual prose to fill the gap, the author checks that new prose's own claim against the actual failure mode it now describes — the same discipline applied to a code fix checked against the criterion it closes — before treating the finding as closed; a `codex-reviewer` or QA re-pass reading the reworked prose applies the same check. Adding words that name the right topic is not the same as adding words that are true of it. When the new sentence claims to hold across several branches at once — a disjunction, an "all N cases" claim, a generic replacement sentence a sweep splices into many files — the author enumerates those branches one by one in the hand-off and records, per branch, what was checked and whether the claim held; "re-read it" without the enumeration is not the check (extended 2026-09-13).
- **Why**: In T-1135, round 1's Major named a runbook's missing prerequisite — the reader may lack the Claude Code CLI, installed or authenticated. Round 2's rework closed it with a new sentence distinguishing "Not logged in" as a network-denial symptom from an authentication problem, but that sentence asserted both the not-installed and not-authenticated cases surface identically as "Not logged in" — false for the not-installed case, where the binary is absent, the shell returns a command-not-found-shaped error, and the application's own "Not logged in" message is never reached. This is a second, independent `docs-overclaim` instance on the same runbook paragraph — not the round-1 defect recurring, but a fresh one the round-2 fix itself introduced — and it surfaced because the `rework-digest.sh` same-class-2 mechanism correctly fired again on this never-pre-committed runbook component.
- **How to apply**: When a rework's fix for a "finding not stated" defect is new explanatory prose rather than a code change, before marking the finding closed, test the new sentence's own claim against each branch it now names (here: not-installed, installed-but-unauthenticated, authenticated) and confirm the stated symptom is what that branch actually produces; a same-class-2 flag on this kind of never-dropped prose component is read as a fresh instance to check, not a repeat to wave through. The enumeration is the deliverable, not the reading: on 2026-09-12/13 this rule was already in the engineer playbook and the class still recurred five times across two tasks (a generic sweep sentence asserting a resolver branch a script lacks; a shared refusal sentence asserting a cause for five reason tokens; a disjunction false for an unresolvable `--ref`; a README remedy clause false for the repo-level route) — each caught by QA or the cross-provider review reading one branch the author had not listed. Listing the branches in the hand-off is what makes the missing one visible before review (source: `.shell-team/retros/2026-09-13-codex-b-ticket.md`).

## 2026-09-12 — A pattern-based finding is reported by location and pattern name, never by quoting the flagged bytes verbatim into a record the same checker scans
- **Category**: security-pii
- **Applies-to**: all
- **Scope**: loop
- **Status**: active
- **Source**: .shell-team/retros/2026-09-12-codex-both-gates.md
- **Rule**: When any role reports a finding from a pattern-based checker (a PII-shape checker, a secret-shape checker), the report names the file, line, and pattern id, and does not quote the flagged bytes themselves verbatim — especially when the report itself lands in a file the same checker scans (a verdict, a board line, an intervention record), where a verbatim quote reproduces the very shape being flagged and adds a new site the remediation must also fix; and because the prose half of this rule has not held on its own, the writer runs that same checker — in whichever mode detects the shape in question, opt-in modes included — against every record file it wrote or edited in the round before handing off, and treats a hit there as its own defect to fix in place, not as a finding to report onward.
- **Why**: In T-1135, QA's round-2 verdict quoted the `check-pii-shapes.sh` `home-path` finding's flagged fragment directly into the verdict text (`FINDING pattern=home-path path=.shell-team/provenance/T-1135.md line=69`, with the matched substring reproduced alongside it), and round 3's own-run confirmation quoted it again; by the time remediation was complete, the flagged shape had propagated to three sites rather than the one the original checker named, each needing its own fix and re-check. The class recurred in the next cycle (2026-09-14/15) at least four times across QA, the cross-provider reviewer, the engineer and the orchestrator, with this entry already active in the playbook blocks: a shape the shipped checker detects only in an opt-in mode (a tracker-key-like token) was quoted into a review record, a provenance file and a hand-off, and a same-class-2 fired on it — the checker could have caught every one of those sites, so the gap was that no writer ran it against the record they had just written.
- **How to apply**: When writing up a pattern-based finding, cite `pattern=<id> path=<file> line=<n>` and the checker's own exit/summary line, and describe the shape in words if description is needed (e.g. "an illustrative placeholder matching the home-path pattern"); do not paste the matched substring itself into the write-up, especially into any file the same checker will later scan. Before hand-off, run the shape checker over the files written or edited this round with the opt-in shapes the task's records could carry switched on (for `check-pii-shapes.sh`, the tracker-key mode via its documented environment variable); a clean run is part of the hand-off, and a hit is fixed before the hand-off is written.

## 2026-09-13 — A frozen check line that asserts a bracketed literal uses a fixed-string or escaped-ERE grep, never basic regex, so a `[` in the literal cannot turn the line into a broken command
- **Category**: verification-discipline
- **Applies-to**: pm-spec, engineer
- **Scope**: loop
- **Status**: active
- **Source**: .shell-team/retros/2026-09-13-codex-b-ticket.md
- **Rule**: When a `- check:` line (or any frozen verification command) asserts the presence or absence of a literal that contains `[`, `]`, `.`, `*`, `+`, `?`, `(`, `)`, `|`, `{`, `}`, `^` or `$` — a type annotation such as `Mapping[str, Any]`, a shell array expansion, a regex quoted from source — the author greps it as a fixed string (`grep -F`, `git grep -F`) or as an extended regex with every metacharacter escaped, never as a bare basic-regex pattern; the freeze-time dry-run and the pre-implementation live run are the moments this is checked, because a line that cannot match its own literal is broken as a command, not a legitimate FAIL, and its repair after the freeze is a class-M re-freeze that costs a human GO wherever no standing grant exists.
- **Why**: On b run #4 (an adopting repository driven from a Codex CLI host, 2026-09-13) a pm-authored, frozen criterion asserted that `Mapping[str, Any]` appeared in a source file and grepped it with basic regex; the brackets were read as a character class, the literal never matched, the line's later step short-circuited to `exit 127`, and the loop stopped for a class-M re-freeze with a human GO (the run's one in-loop host input beside the start prompt). The correct spellings existed all along; the defect was authoring, not tooling.
- **How to apply**: Before freezing, scan every `- check:` line for grep/git-grep calls without `-F`/`-E`/`-P` whose pattern carries a regex metacharacter; rewrite to `-F` (when the whole literal is fixed) or to `-E` with escapes. QA and the cross-provider review treat a bare-BRE bracketed literal in a check line as a broken-as-command finding even when the line happens to pass, because it passes for the wrong reason.

## 2026-09-16 — A diagnostic line added to a script with more than one exit path is placed and tested at every dispatch point, never only at the one the finding named
- **Category**: verification-discipline
- **Applies-to**: pm-spec, engineer
- **Scope**: loop
- **Status**: active
- **Source**: .shell-team/retros/2026-09-16-loop-signals-honest.md
- **Rule**: When a change adds a notice, warning or other diagnostic line to a script whose contract is one refusal line and nothing else on a failure path, the spec enumerates every mode and exit point of that script as a fixture matrix and the implementation emits the diagnostic at a point each mode reaches only after its last refusal — a fix that closes the reported site alone leaves the same defect one dispatch point over.
- **Why**: In one rename task the same class fired in three consecutive cross-provider review rounds — first on the resolver path, then on the validator's direct invocation, then in the validator's verify mode — because each fix closed the site the finding named and the next round found the mode nobody had enumerated; the pre-commitment's trigger fired and the operator granted an extra round before the class was closed structurally at every exit.
- **How to apply**: At spec time, list the script's modes and exit points and require one fixture per mode asserting the exact stderr line count on both a refusal and a success; at implementation time, emit the diagnostic once per mode immediately before that mode's success exit, and mutation-test by moving it earlier in one mode and watching that mode's refusal case emit two lines.

## 2026-09-16 — A rename or removal task inventories the artifacts the plugin generates into adopter-side, untracked locations, not only the files under version control
- **Category**: process
- **Applies-to**: pm-spec, tech-lead
- **Scope**: loop
- **Status**: active
- **Source**: .shell-team/retros/2026-09-16-loop-signals-honest.md
- **Rule**: When a task renames, removes or reshapes anything the plugin generates into an adopter's own environment — a generated agent file in an ignored output directory, a snippet adopters copy into their own configuration, telemetry already written under the old name — the spec's blast radius names those adopter-side artifacts as an input class of their own, with a migration path or a stated limit for each, because an inventory of tracked files cannot see them.
- **Why**: In one rename task every cross-provider review round surfaced one more adopter-side artifact the tracked-file inventory had missed — a second telemetry reader, a stale generated agent file that made the sync checker fail after an upgrade, and a partial regeneration that deleted that file without writing its replacement — and each cost a review round.
- **How to apply**: Before freezing a rename or removal spec, walk the repository's generators and templates and list every output that lands outside version control or inside an adopter's own copy; for each, decide and record one of: migrate automatically, document a manual step, or state the limit; then add a fixture for every automatic migration and a documentation criterion for every stated limit.

## 2026-09-19 — Release-level verification cost is priced by the release's cumulative deliverable class
- **Category**: process
- **Applies-to**: pm-spec
- **Scope**: maintainer
- **Bound-in**: CONTRIBUTING.md
- **Status**: active
- **Source**: .shell-team/retros/2026-09-19-docs-two-hosts.md
- **Rule**: When the cumulative diff since the last release-level sweep touches only documentation (`docs/`, `README*`, changelog files) and nothing under `bin/`, `agents/`, `skills/`, `templates/` or `.github/`, the release procedure's mechanism-class two-arm sweep (a full-population freeze/blast-radius diff run once per release) is scoped to the specs whose `- check:` lines read the touched paths — derived with `git grep -l` over the check-line grammar against the specs directory — or is omitted outright with the reason recorded on the release record, rather than launched against the full spec population as the procedure's own default; this extends the 2026-08-13 lesson "Freeze-time verification cost is priced by the task's deliverable class" (`.shell-team/lessons.md`) from a single task's acceptance-criteria verification shape to the release procedure's own sweep, because the same deliverable-class judgment (mechanism change vs. wording-only/docs-only) prices both.
- **Why**: In one release cycle, the cumulative change since the prior sweep was two documentation tasks describing an already-shipped mechanism, with no file under `bin/`, `agents/`, `skills/`, `templates/` or `.github/` touched; the release-level two-arm sweep nonetheless launched against the full spec population before the operator halted it mid-run on observing that a mechanism the loop's own development-time review and QA gates had already verified was about to be re-verified a second time at release, at a cost judged to exceed the value of running it. The 2026-08-13 lesson already prices this exact judgment at the single-task level; nothing carried the same pricing rule to the release-level sweep, so the release procedure defaulted to full population and needed a one-off human interruption rather than a standing rule to reach the cheaper scope.
- **How to apply**: Before launching a release's two-arm mechanism-class sweep, diff the release's cumulative change set against `bin/`, `agents/`, `skills/`, `templates/`, `.github/`; if none of those paths appear, derive the affected spec population per changed path with `git grep -l -E '^[[:space:]]*- check:.*<changed-path-fragment>' -- <specs dir>` and run the sweep against that population only, or record on the release line the decision to omit it and why, rather than launching against the full population by default. This repository's Applies-to enum carries no token for the coordinating session or orchestrator role that actually decides whether to run the release-level sweep — pm-spec is the nearest existing role, so this entry is bound by hand into `CONTRIBUTING.md`'s release procedure, the same way the 2026-08-11 stacked-batch-GO entry is bound into `agents/scrum-master.md`.

## 2026-09-19 — The adopter-docs gate asks whether one shipped document names a mechanism, not whether every shipped document naming it stays current
- **Category**: process
- **Applies-to**: pm-spec, tech-lead
- **Scope**: loop
- **Status**: active
- **Source**: .shell-team/retros/2026-09-19-docs-two-hosts.md
- **Rule**: When a `user-visible: yes` spec's Summarized sources or Blast radius section names more than one shipped document that already describes the mechanism the spec is changing or extending, but the spec's own edit list touches only some of them, the spec lists every such document at freeze time and records, for each one this task does not edit, either "updated in this task" or a filed follow-up issue naming it explicitly — so a shipped document describing a mechanism never stays stale across releases merely because the task that last touched the mechanism happened to edit a different document.
- **Why**: The 2026-08-11 lesson "A spec for a user-visible capability carries an adopter-facing-documentation acceptance criterion as a freeze-time blocker" (`.shell-team/lessons.md`) already requires a user-visible spec to ship adopter-facing documentation as a freeze-time blocker, and that gate is satisfied the moment one such document exists and is edited; it is forward-only by its own originating spec's stated Non-goals (no retrofit of any existing spec) and only asks whether documentation exists for the capability shipping now, never whether other already-shipped documents describing the same mechanism are being kept current. A Codex-CLI-host mechanism shipped in an earlier release documented itself in exactly one adopter-facing file; two other shipped documents describing the same host-parity behavior — the project README and a distribution/workflow guide — went unedited across three subsequent releases until a dedicated two-task sprint caught up all three at once, a gap the adopter-docs gate had no way to detect because it never re-examines a document this task was not the one to touch.
- **How to apply**: At spec-completion self-check, when a `user-visible: yes` spec's Summarized sources or Blast radius already lists more than one shipped document describing the mechanism under change, do not let satisfying the adopter-docs gate's single-document requirement close the question: enumerate every such document by path, and for each one the spec does not edit, either add it to this task's edit list or record a named follow-up issue against it before freezing. Tech-lead's design pass and pm-spec's freeze sweep both treat an enumerated-but-undecided document as an open freeze-time finding, the same way an unaddressed adopter-facing-documentation gap already is under the 2026-08-11 entry.

## 2026-09-19 — A spec that hands an inventory to verifying-role judgment names the round it becomes a systematic sweep, not a per-finding response
- **Category**: verification-discipline
- **Applies-to**: pm-spec, engineer, qa-verifier
- **Scope**: loop
- **Status**: active
- **Source**: .shell-team/retros/2026-09-19-docs-two-hosts.md
- **Rule**: When a spec deliberately declines to make some inventory — a parity-claim-word list, a runbook-entailment table, a host-qualifier sweep across every host-behavior sentence — a mechanical pre-check duty for the engineer (because a fixed word list or grep would false-positive on legitimate uses, or would freeze a vocabulary the task has no authority to close) and instead leaves it to a verifying role's reading judgment, the spec names, at freeze time, the round in which that inventory runs as a full systematic sweep rather than as an ad-hoc response to each individual finding.
- **Why**: In one task, the same meta-class of finding — a host-behavior sentence missing its host qualifier — fired once per round across four consecutive independent gating rounds (QA round 1, QA round 2, review round 1, review round 2), each catching one more sentence the previous round's fix had not touched, before round 5 finally had the engineer, QA and the cross-provider reviewer each independently run the full inventory across the document and converge on the same result. The spec had already identified the inventory as design content — it carried both a Runbook-entailment table and a Parity-claim-word inventory section — and had already reasoned about why it could not be a mechanical engineer pre-check, but it never said when the judgment-based version of that inventory was supposed to run as a sweep instead of a one-sentence-at-a-time response, so nobody ran it systematically until four rounds of accumulated cost had already been paid.
- **How to apply**: At spec freeze time, for every inventory a spec deliberately assigns to a verifying role's reading judgment rather than to a mechanical check, add a line naming the trigger for running it as a full sweep — most cheaply, "the second finding in this meta-class converts the next round from a point-fix to a full-document inventory by whichever role receives it." Engineer, qa-verifier and the cross-provider reviewer treat a second same-meta-class finding within one task as that trigger firing even where the spec's own line is silent, and record in the round's hand-off that the inventory was run as a sweep rather than as a fix to the one flagged sentence.

## 2026-09-20 — A checker rebuilt to close fence, scope, or position boundary defects carries a token-family-to-boundary-variable inventory, not only a per-token fixture audit
- **Category**: verification-discipline
- **Applies-to**: pm-spec, engineer, qa-verifier
- **Scope**: loop
- **Status**: active
- **Source**: .shell-team/retros/2026-09-20-codex-wait-b-run7.md
- **Rule**: When a task rebuilds or extends a checker that enforces a fence, scope, or positional boundary — the region a value must sit inside, the marker pair that opens and closes it, or the ordering a set of tokens must respect — the spec and its verification add, beside the per-token fixture-completeness audit, an inventory listing every token family the checker recognizes against which boundary variable actually bounds it in the code, checked as its own dogfood criterion, because a per-row fixture audit confirms each row already in the table is correct but cannot detect a token family that has no row in the table at all.
- **Why**: In one checker rebuild, a cross-provider review round found a boundary variable that closed only on the file's next heading with no upper bound at the intent block's own end marker, so two of the checker's recognized token families were bounded correctly while two others were not — live-reproduced as a zero-byte, exit-0 acceptance of a value placed entirely outside the region the checker's own header comment claims to enforce. Both the same round's coverage-gap sweep and an earlier round's full table-by-fixture inventory had already checked, for every row already in the boundary table, that a fixture existed and passed for it; neither audit asked whether every recognized token family had a row in the table at all, so the missing binding was invisible to a per-row completeness check by construction, not by a gap in running it.
- **How to apply**: When freezing or reviewing a checker generation whose stated Goal names a boundary it enforces, build a small table with every token family the checker recognizes down one axis and every boundary variable it maintains across the other, mark which cell each family is actually bound by from reading the code rather than the header comment, and treat any family with no marked cell as an open finding before treating the per-token fixture audit as complete; run this table-building pass as its own step, prior to and distinct from, the fixture-completeness sweep.

## 2026-09-20 — An unbounded population job runs inside a disposable clone under a scratch directory, never as a background job aimed at the working tree it would edit
- **Category**: process
- **Applies-to**: engineer, qa-verifier
- **Scope**: loop
- **Status**: active
- **Source**: .shell-team/retros/2026-09-20-codex-wait-b-run7.md
- **Rule**: When a spec calls for a verification job whose input population is not bounded by the current task's own diff — a full read-set regression, a full-population two-arm blast-radius sweep, or any check spanning the whole spec or task corpus rather than a named, small set of paths — the job is launched inside a disposable clone under a scratch directory, never as a background job aimed directly at the working tree the loop is editing, because a single long-running spec within an unbounded population can stall the job silently and the role that launched it has no mechanism to stop or diagnose it once it is running.
- **Why**: In one task, a per-task full-population regression was launched as a harness-tracked background job against the real working checkout; it stalled silently on the very first spec in its derived read-set, producing an empty log for fifty minutes before the coordinating session intervened to abandon it, and the launching role — despite disclosing the stall in its own hand-off — had no tool available to stop or inspect the stalled job itself. No file under version control was corrupted this time only because the stall happened before the job produced any output; the same silent stall occurring further into a job's run could leave a half-written result indistinguishable from a completed one.
- **How to apply**: Before launching any verification job whose population is not bounded by the current task's own diff, clone the repository into a disposable location under a scratch directory and run the job there instead of against the working tree; reserve launching directly against the working tree for jobs whose input set is small, named, and known in advance. If a spec cannot state its population's upper bound at freeze time, treat that as the trigger for routing the job through a disposable clone rather than deciding case by case at launch time.
