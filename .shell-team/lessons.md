...
- **Why**: In T-1135, QA's round-2 verdict quoted the `check-pii-shapes.sh` `home-path` finding's flagged fragment directly into the verdict text (`FINDING pattern=home-path path=.shell-team/provenance/T-1135.md line=69`, with the matched substring reproduced alongside it), and round 3's own-run confirmation quoted it again; by the time remediation was complete, the flagged shape had propagated to three sites rather than the one the original checker named, each needing its own fix and re-check.
- **How to apply**: When writing up a pattern-based finding, cite `pattern=<id> path=<file> line=<n>` and the checker's own exit/summary line, and describe the shape in words if description is needed (e.g. "an illustrative placeholder matching the home-path pattern"); do not paste the matched substring itself into the write-up, especially into any file the same checker will later scan.
