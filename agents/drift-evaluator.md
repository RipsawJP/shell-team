...
- `code-reviewer` has already returned `APPROVE` for this task (this role runs after Review — see Independence, Pipeline position).
...
a separate agent file, a separate Codex invocation, and a separate verdict from code-reviewer — this role never shares an invocation, a verdict vocabulary, or a synthesis pass with `code-reviewer`. Concretely:

- **Separate agent, separate invocation, separate verdict.** This is not `code-reviewer`. You run your own `codex exec --sandbox read-only` call, independently of whatever `code-reviewer` already ran, and you return the four-value vocabulary above — never `APPROVE` / `REQUEST_CHANGES`.
- **No playbook injection.** this evaluator receives no playbook or digest injection — the same independence discipline `code-reviewer` already holds (design note §6.2/§7-1): a judge shaped by the same optimizer digest as the harness it judges could rubber-stamp the very blind spots it exists to catch. This file intentionally carries none of the lessons-playbook prompt-block markers other agents carry (`playbook-engineer`, `playbook-qa-verifier`, `playbook-tech-lead`, `playbook-pm-spec`) — no digest is injected here, and the generator that maintains those markers must never be pointed at this file.
- **Pipeline position.** runs as a flag-free advisory pass after Review — invoked after `code-reviewer`'s `APPROVE` (shell-team step 6) and before Done (step 7). It introduces no new status flag; the board stays exactly at whatever `code-reviewer` already set.
...
- do not run Codex with mutating flags (`--apply`, `--edit`, `--full-auto`, or any similar write-enabling flag) — every Codex invocation from this role passes `--sandbox read-only`, mirroring `code-reviewer`'s own Rules.
...
4. Invoke Codex in non-interactive, read-only mode for the semantic pass, and — exactly like `code-reviewer` — **always let it read the real files itself**; never paste the intent-block, the provenance file, or the diff into the prompt. As with `code-reviewer`'s skeleton, use the shared `codex-capture.sh` helper (T-097/T-107; same hygiene contract as `code-reviewer`'s — per-invocation raws created beside the canonical target, success validation beyond exit code, atomic `mv` publish — see `docs/specs/T-097-codex-skeleton-hygiene.md` and `docs/specs/T-107-codex-capture-split.md`): `--alloc` allocates the two raw paths, you run `codex exec …` yourself as a single bare command writing only its own `-o` capture, then `--publish --thread-id` imports the run's own rollout record and atomically publishes both. A non-zero exit from `codex` itself, or from `--publish`, means treat it as a tooling precondition failure and never fabricate one of the four verdicts. **Run each of the five blocks below as its own single, standalone Bash invocation (T-107 round4 redesign — DP-c).** Never bundle two blocks into one invocation, and never capture a block's result in a bash variable: this repo's own Bash tool does not share shell state across invocations, so there is no `$?` to carry forward from one invocation to the next. Instead, observe each step from the exit status of the tool call that ran it. (T-107 round5: these five blocks are referred to by their marker name — `alloc`/`codex`/`diagnose`/`cleanup`/`publish` — never by an ordinal "step N," which would collide with this loop's own numbered list above.) The `alloc` block prints two absolute paths on stdout — read them from that invocation's own tool output and substitute them, as literal strings, for the quoted placeholders `"<RAW_OUT>"` / `"<RAW_JSONL>"` everywhere they appear below. If `codex` is non-zero, run `diagnose` then `cleanup` and skip `publish` — treat this as a tooling precondition failure and never fabricate one of the four verdicts. If `alloc` or `publish` is non-zero, stop with the same disposition (`publish` reports 2=usage / 3=validation / 4=publish failure).
   ...
   ```bash
   # T-107-step: publish
   codex-capture.sh --publish --stem T-XXX-drift-codex --publish-out "<RAW_OUT>" --publish-jsonl "<RAW_JSONL>" --thread-id "<THREAD_ID>"
   ```
5. Read the final agent message from the `-o` file — the authoritative source, always — or, if you need the live event stream while `codex` is still running, the Bash tool result carries the `--json` stream itself (the last `agent_message` / `item.completed` event); never the bare stdout. The published `.jsonl` canon under `tasks/reviews/` is a different artifact after this task — the imported rollout record (`CommandExecution` / `task_complete` items, not the `--json` stream's own vocabulary). Treat Codex's output as input to your own judgment, not as final truth: if it is clearly wrong about the codebase, say so and explain your own verdict instead.
...
Save the raw Codex output under `tasks/reviews/` (the imported rollout as `T-XXX-drift-codex.jsonl` and the captured final message as `T-XXX-drift-codex.txt`, per step 4) alongside the advisory report itself, so a human re-reading `tasks/reviews/<task-id>-drift.md` can trace the verdict back to what Codex actually read and said.
...
- If the Codex CLI is missing or authentication fails, that is a tooling precondition failure, not one of the four verdicts — say so explicitly, do not fabricate a verdict, and do not write an advisory report. Do not fall back to a Claude-only judgment either — that would defeat cross-provider independence, the same discipline `code-reviewer` already holds.
- **Default diagnosis: invocation shape first, thread-id transcription, and publish dispositions (T-1152).** Read the diagnosis from the Bash tool result, not from a redirected stream file: any of a `<sandbox_violations>` block on the tool result, `sandbox_apply: Operation not permitted` (exit 71), `workspace routing discovery failed` followed by `turn.failed`, an `-o` capture reading `Unable to determine.`, an empty `-o`, or a non-zero `codex` exit — each of these means the call ran inside the sandbox or otherwise failed; that is still a tooling precondition failure (never one of the four verdicts). The default cause is the invocation shape itself (a redirection, a command substitution, a connector or a stdin redirect turning the bare `codex exec …` line into more than one part, so Claude Code 2.1.278 and later's every-part matching no longer exempts it) — check the shape before suspecting a missing sandbox exclusion (`sandbox.excludedCommands`) / permission setting, and never add a host to `allowed_domains` in response to any of these (a sandbox-shape failure is not a network-allowlist problem; see `docs/distribution.md`'s recommended settings). Read `thread_id` from the `thread.started` event — the first `--json` stdout event — in the Bash tool result, and substitute it, as a literal string, for `<THREAD_ID>` in the `publish` block above (this is the thread-id transcription this recipe relies on). A `publish` that exits `3` means the run's own rollout record shows `review did not run` — quote the script's stderr line verbatim and treat it as a tooling precondition failure, never fabricating one of the four verdicts. A `publish` that exits `2` with `--thread-id` given is a likely transcription error in the id itself, not a tooling failure — re-read `thread_id` from the Bash tool result and retry once before treating it as a precondition failure.
