...
**Route-back**: on `REQUEST_CHANGES`, route the findings back to the spec's own author, resolved through the `specify` decision recorded on the task's board entry, never assumed — `pm-authored` routes to `pm-spec`; `operator-authored` routes to the author (the coordinating session that wrote the spec).
