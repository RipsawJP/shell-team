...
- **Price any release-level verification sweep by the release's cumulative deliverable class.** A maintainer who runs a full-population two-arm re-verification of the frozen acceptance criteria before a release (a base-versus-head `check-acs` diff over every spec) scopes it by what the release actually changed: when the cumulative diff since the previous release touches only documentation and the changelogs — nothing under `bin/`, `agents/`, `skills/`, `templates/` or `.github/` — the sweep runs only over the specs whose `- check:` lines read the touched paths, or is omitted with the reason recorded in the release pull request. The mechanism those documents describe was verified by the loop's own gates when it shipped; re-verifying it at a documentation-only release measures staleness in merged specs, not regressions.
